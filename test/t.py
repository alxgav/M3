...
def extract_article_number(article_field) -> list:
    return [re.split(r"[-BA]", article_field)[0], re.split(r"[-BA]", article_field)[1]]
...
def extract_selected_fields(file_path, selected_indices, field_names):
    ...
    with open(file_path, "r", encoding="iso-8859-1") as f:
        ...
        for item in lines:
            ...
            material_entries = [
                entry for entry in store if article[0] in entry["Artikel"]
            ]
            ...
            data_work.append(labeled_fields)
    return data_work
...
#     # artikel = re.split(r"[-BA]", item["Article"])[1]
#     for material in item["materials"]:
#         print(re.split(r"[-BA]", material["Artikel"]))
